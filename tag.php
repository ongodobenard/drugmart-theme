 ...
 * Styled to match the custom "Blog" page template.
 */
get_header();

$bg_tag_obj  = get_queried_object();
$bg_tag_name = isset($bg_tag_obj->name) ? $bg_tag_obj->name : '';
$bg_tag_desc = isset($bg_tag_obj->description) ? $bg_tag_obj->description : '';
$bg_blog_url = home_url('/blog');

/* ── SIDEBAR DATA (identical to blog.php) ── */
...
.bg-tag-pill.active { background: var(--bg-blue); color: #fff; }
...
<div class="bg-wrap">
  ...
  <!-- HERO -->
  <div class="bg-hero">
    <span class="bg-tag">Tag</span>
    <h1 class="bg-title-main">Posts tagged <span>"<?php echo esc_html($bg_tag_name); ?>"</span></h1>
    <?php if ($bg_tag_desc): ?>
      <p class="bg-hero-desc"><?php echo esc_html($bg_tag_desc); ?></p>
    <?php else: ?>
      <p class="bg-hero-desc">Browsing all articles tagged "<?php echo esc_html($bg_tag_name); ?>" from Family Drugmart Kenya.</p>
    <?php endif; ?>
  </div>

  <!-- BACK TO BLOG -->
  <div class="bg-filter-row">
    <a href="<?php echo esc_url($bg_blog_url); ?>" class="bg-filter-pill">
  ...
  <div class="bg-layout">
    ...
    <div>
      ...
      <div class="bg-grid">
        ...
        <?php endwhile; else: ?>
          <div class="bg-empty">No posts found with this tag yet.</div>
        <?php endif; ?>
      </div>
      ...
      <?php
      global $wp_query;
      $bg_total = $wp_query->max_num_pages;
      $bg_paged = max(1, (int) get_query_var('paged'));
      if ($bg_total > 1):
      ?>
      <div class="bg-pagination">
        <?php if ($bg_paged > 1): ?>
          <a href="<?php echo esc_url(get_pagenum_link($bg_paged - 1)); ?>" class="bg-page-btn prev">
        ...
        <?php if ($bg_paged < $bg_total): ?>
          <a href="<?php echo esc_url(get_pagenum_link($bg_paged + 1)); ?>" class="bg-page-btn next">
    ...
    <aside class="bg-sidebar">
      ...
      <div class="bg-widget">
        ...
        <form class="bg-search-form" action="<?php echo esc_url($bg_blog_url); ?>" method="get">
          <input type="text" name="bsearch" class="bg-search-input" placeholder="Search articles…">
          ...
          <div class="bg-tag-cloud">
            <?php foreach ($bg_tags as $tag): ?>
              <a href="<?php echo esc_url(get_tag_link($tag->term_id)); ?>"
                 class="bg-tag-pill <?php echo ($bg_tag_obj instanceof WP_Term && $tag->term_id === $bg_tag_obj->term_id) ? 'active' : ''; ?>">
